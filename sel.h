Matrix createLocalK(int element,mesh &m){
    ...
    float j = m.getParameter(CONS_E), k = m.getParameter(CONS_A),
    l = m.getNode(element+1).getX() - m.getNode(element).getX();
    
    row1.push_back((j*k)/(2*l)); row1.push_back(-(j*k)/(2*l));
    row2.push_back(-(j*k)/(2*l)); row2.push_back((j*k)/(2*l));
